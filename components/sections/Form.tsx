...
export const MyForm: FC<{ title?: string }> = ({ title }) => {
	...
	return (
		<div className='px-4 pt-8 md:p-8 xl:p-12 2xl:px-[70px] 2xl:py-[50px] flex flex-col gap-[37px] max-w-4xl mx-auto'>
			<div className='flex flex-col gap-[9px]'>
				<h3 className='text-3xl xl:text-[45px] font-bold xl:leading-[55px] text-gray-800'>
					{title && title ? title : 'Форма обратной связи'}
				</h3>
				<p className='font-medium text-md md:text-xl text-gray-800'>
			...
			<form onSubmit={form.onSubmit(values => handleSubmit(values))}>
				<div className='flex flex-col gap-4 md:gap-[23px] formWrapper'>
					<TextInput
						withAsterisk
						placeholder='Имя'
						key={form.key('name')}
